...
이 프로젝트는 [Clerk 공식 통합 가이드](https://clerk.com/docs/guides/development/integrations/databases/supabase)와 [Supabase Clerk 통합 문서](https://supabase.com/docs/guides/auth/third-party/clerk)를 기반으로 구현되었습니다.

**3-1. Clerk에서 Supabase 통합 활성화**

1. [Clerk Dashboard](https://dashboard.clerk.com)에 로그인
2. **Setup** → **Integrations** → **Supabase** 메뉴로 이동
3. **"Activate Supabase integration"** 클릭
4. **Clerk domain** 복사 (예: `https://your-app-12.clerk.accounts.dev`)
   - 이 도메인은 다음 단계에서 사용합니다

**3-2. Supabase에서 Clerk를 제3자 인증 제공자로 추가**
...
**커스터마이징:**
특정 텍스트나 에러 메시지를 커스터마이징하려면 `lib/clerk/localization.ts` 파일을 수정하거나, `app/layout.tsx`에서 직접 커스텀 로컬라이제이션을 적용할 수 있습니다:
...
- **`/storage-test`**: Supabase Storage 파일 업로드 테스트
...
- **`lib/supabase/clerk-client.ts`**: Client Component용 Supabase 클라이언트 (Clerk 인증 포함)
- **`lib/supabase/server.ts`**: Server Component/Server Action용 Supabase 클라이언트 (Clerk 인증 포함)
- **`lib/supabase/service-role.ts`**: 관리자 권한 Supabase 클라이언트 (RLS 우회)
- **`lib/supabase/client.ts`**: 공개 데이터용 Supabase 클라이언트 (인증 불필요)

### Supabase 클라이언트 사용법

이 프로젝트는 Clerk와 Supabase의 네이티브 통합을 사용합니다. 환경에 따라 적절한 클라이언트를 사용하세요:
...
- 자세한 내용은 [Clerk 공식 통합 가이드](https://clerk.com/docs/guides/development/integrations/databases/supabase)와 [Supabase Next.js 퀵스타트](https://supabase.com/docs/guides/getting-started/quickstarts/nextjs)를 참고하세요
